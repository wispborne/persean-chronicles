...
class Telos3LandingDialog(
    ...
    override fun pages() = object : PagesFromJson<Telos3LandingDialog>() {
        ...
        override fun onPageShownHandlersByPageId() = mapOf(
            ...
            "7-shipyard-itesh" to
                    {
                        dialog.visualPanel.showFleetMemberInfo(
                            game.settings.getVariant("wisp_perseanchronicles_itesh_Standard")
        ...
        override fun optionConfigurator() = { options: List<IInteractionLogic.Option<Telos3LandingDialog>> ->
            options.map { option ->
                when (option.id) {
                    ...
                    "return-to-orbit" -> option.copy(
                        onOptionSelected = { navigator.close(doNotOfferAgain = false) }
                    )
